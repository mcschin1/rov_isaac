...
Usage:
    python rov_pipeline_rl_v7.py --train                 # train PPO from scratch
    python rov_pipeline_rl_v7.py --evaluate               # run 1 full eval episode + plots
    python rov_pipeline_rl_v7.py --train --evaluate        # both, back to back
"""
...
class PipelineTrackingEnv:
    ...
    def __init__(self, scene, dynamics, n_envs, max_episode_steps,
                 ...
                 standoff_target=1.0, standoff_track_target=1.2, k_track=10.0,
...
def train(total_timesteps=6_000_000, n_envs=64, rollout_len=256,
          lr=3e-4, clip_ratio=0.2, train_epochs=10, minibatch_size=4096,
          vf_coef=0.5, ent_coef=0.01, max_grad_norm=0.5, gamma=0.99, lam=0.95,
          log_every=1, seed=0):
    ...
    for update in range(1, n_updates + 1):
        ...
        if update % log_every == 0:
            ...
            print(f"update {update}/{n_updates} | steps {global_step} | "
                  ...
                  f"flip {flip_rate:.2f} | gap_violation {gap_viol_rate:.2f} | "
            ...
            # preferred over a safer, slightly-less-successful one.
            if len(finished_success) >= 50:
                score = succ - 0.5 * flip_rate - 0.5 * gap_viol_rate
                if score > best_score:
                    best_score = score
                    best_update = update
                    save_checkpoint()
...
def plot_training_curves(csv_path):
    ...
    axes[0].set_title("PPO training curves -- ROV pipeline tracking (v7)")
